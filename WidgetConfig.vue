  ...
  <div class="widget-config">
    <el-form label-suffix="：" v-if="this.data && Object.keys(this.data).length > 0" size="small">
      <el-form-item label="类型">
        <el-select v-model="data.type" placeholder="请选择类型" @change="handleChangeType">
      ...
      <el-form-item label="栅格" v-if="!data.subfield && !['group'].includes(data.type)">
        <el-input-number
          v-model="data.span"
          controls-position="right"
          placeholder="栅格"
...
export default {
  ...
  computed: {
    getComponent() {
      ...
      if ([undefined, 'input', 'url'].includes(type)) result = 'input';
  ...
  methods: {
    ...
    getConfigByType(type) {
      return new Promise((resolve, reject) => {
        ...
        // TODO: Expected the Promise rejection reason to be an Error.
        // reject();
      });
    }
  }
